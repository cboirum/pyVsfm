...
% Argument sanity checking
if (cam_names_sz(2) && cam_vec_sz(2) ~= cam_names_sz(2))
    display('Error: Your camera vector and naming vector are of different sizes. Cannot continue.');
    return;
end

% Make sure we are calibrating two cameras or more
if (cam_vec_sz(2) < 2 || rotcam_sz(2) < 2 || cam_names_sz(2) < 2)
...
% Sort the camera list into the right order
camera_vec = sort(camera_vec);

display(['The origin camera will be camera ' num2str(camera_vec(1)) '.']);

for ii = 2:1:cam_vec_sz(2)
    temp_vec = [camera_vec(1) camera_vec(ii)];
    rotcam_temp_vec = [rotcam(1) rotcam(ii)];
    ...
    if (~exist('cam_names','var'))
        ...
        cam0_names = cam_names(1);
        cam1_names = cam_names(ii);
    end
    
    display(['Stereo calibrating cameras ' num2str(camera_vec(1)) ' and ' num2str(camera_vec(ii)) '...']);
    if(batch)
